...
function run_scenario(file_path::String, tag::String)
    ...
    for e in 2:K, n in N[e], t in Tset
        @constraint(model, x[e, n, t] <= 100000000.0 * y[e, n, t])
    end
    ...
    for e in 1:K, n in N[e], t in Tset
        @constraint(model, I[e, n, t] <= 0)
    end

    for e in K+1:E_max, n in N[e], p in P, t in Tset
        @constraint(model, J[e, n, p, t] <= 0)
    end
    ...
    production_3_2 = [value(x[3, 2, t]) for t in Tset]
    production_2_2 = [value(x[2, 2, t]) for t in Tset]
    production_2_1 = [value(x[2, 1, t]) for t in Tset]
    production_2_3 = [value(x[2, 3, t]) for t in Tset]
    production_2_4 = [value(x[2, 4, t]) for t in Tset]
    ...
    #turnover by echelon
    turnover = Dict{Tuple{Int,Int},Float64}()
    total_outflow = Dict{Tuple{Int,Int},Float64}()
    total_inventory = Dict{Tuple{Int,Int},Float64}()


    for e in 1:E_max
        for n in N[e]
            ### 1) Compute total inventory at node (e,n) ###
            if e ≤ K
                # Manufacturing echelons: inventory = I[e,n,t]
                inv_sum = sum(value(I[e, n, t]) for t in 1:num_hours)
            else
                # Downstream echelons: inventory = sum over products p of J[e,n,p,t]
                inv_sum = sum(value(J[e, n, p, t]) for p in P for t in 1:num_hours)
            end
            total_inventory[(e, n)] = inv_sum

            ### 2) Compute total outflow at node (e,n) ###
            out_sum = 0.0

            if e < K
                # Echelons 1..(K-1): outflow via f_up[e,n,m,t]
                for m in outgoing_connections[(e, n)], t in 1:num_hours
                    out_sum += value(f_up[e, n, m, t])
                end

            elseif e == K
                # Echelon K → next (downstream) echelon uses f_down[K,n,m,n,t]
                for m in outgoing_connections[(K, n)], t in 1:num_hours
                    out_sum += value(f_down[K, n, m, n, t])
                end

            elseif (K < e) && (e < E_max)
                # Intermediate downstream echelons: outflow via f_down[e,n,m,p,t] for p=1:2
                for m in outgoing_connections[(e, n)], p in P, t in 1:num_hours
                    out_sum += value(f_down[e, n, m, p, t])
                end

            else
                # e == E_max (last echelon): outflow is external demand d[(E_max,n,p,t)]
                for p in P, t in 1:num_hours
                    out_sum += d[(E_max, n, p, t)]
                end
            end

            total_outflow[(e, n)] = out_sum

            ### 3) Compute turnover ratio, guarding against divide-by-zero ###
            if total_inventory[(e, n)] == 0.0
                turnover[(e, n)] = 0.0
            else
                turnover[(e, n)] = total_outflow[(e, n)] / total_inventory[(e, n)]
            end
        end
    end
    ...
    for e in 1:E_max
        ...
        node_turnovers = [turnover[(e, n)] for n in N[e]]
    ...
    all_turnover_values = collect(values(turnover))
    overall_avg_turnover = isempty(all_turnover_values) ? 0.0 : mean(all_turnover_values)
    ...
    df = DataFrame(
        ...
        Production_3_2_tonnes=production_3_2,
        Production_2_2_tonnes=production_2_2,
        Production_2_1_tonnes=production_2_1,
        Production_2_3_tonnes=production_2_3,
        Production_2_4_tonnes=production_2_4,
    ...
    mkpath("OPEX_baseline_case/results")
    out_file = "OPEX_baseline_case/results/zero_inv_$(tag).xlsx"
