...
function run_scenario(file_path::String, tag::String)
    ...
    set_optimizer_attribute(model, "LogFile", "capex_problem/results/gurobi_log_$(tag).txt")
    ...
    @variable(model, f_up[e in 1:K-1, n in N[e], m in N[e+1], t in 0:maximum(Tset)] >= 0)

    @variable(model, f_down[e in K:E_max-1, n in N[e], m in N[e+1], p in P, t in 0:maximum(Tset)] >= 0)
    ...
    @objective(model, Min,
        ...
        sum(capacity_cost_prod[(K, n)] * cap_prod[K, n] for n in N[K])
    ...
    # inflow_5_5_OPC = [sum(value(f_up[4, m, 5, 1, t])
    #                       for m in incoming_connections[(5, 5)])
    #                   for t in Tset]
    ...
    # 3) Compute overall average turnover across all (e,n)
    overall_avg_turnover = isempty(avg_turnover_per_echelon) ? 0.0 :
                           mean(collect(values(avg_turnover_per_echelon)))
    timestamp = df[!, "timestamp"]

    inflow_5_5_OPC = [sum(value(f_down[4, m, 5, 1, t])
                          for m in incoming_connections[(5, 5)])
                      for t in Tset]

    outflow_2 = Dict{Int,Vector{Float64}}()
    for n in N[2]
        outflow_2[n] = [sum(value(f_up[2, n, m, t])
                            for m in outgoing_connections[(2, n)])
                        for t in Tset]
    end

    # 2) Echelon 3 (uses f_down[K=3, p==n])
    outflow_3 = Dict{Int,Vector{Float64}}()
    for n in N[3]
        outflow_3[n] = [sum(value(f_down[3, n, m, n, t])
                            for m in outgoing_connections[(3, n)])
                        for t in Tset]
    end

    # 3) Node (4,2), just product p=1 (OPC)
    outflow_4_2 = [sum(value(f_down[4, 2, m, 1, t])
                       for m in outgoing_connections[(4, 2)])
                   for t in Tset]

    # 4) Node (5,5), final echelon: external demand as outflow
    outflow_5_5 = [sum(d[(5, 5, p, t)] for p in P) for t in Tset]

    energy_by_node = Dict{Tuple{Int,Int},Vector{Float64}}()
    for e in 2:K
        for n in N[e]
            if haskey(elec_usage, (e, n))
                energy_series = Float64[]  # one value per t
                for t in Tset
                    energy = value(x[e, n, t]) * elec_usage[(e, n)]  # kWh
                    push!(energy_series, energy)
                end
                energy_by_node[(e, n)] = energy_series
            end
        end
    end

    prod_ts = Dict{Tuple{Int,Int},Vector{Float64}}()
    for e in 2:3
        for n in N[e]
            prod_ts[(e, n)] = [value(x[e, n, t]) for t in Tset]
        end
    end
    ...
    for n in N[K]
        CAPEX_OPEX += capacity_cost_prod[(K, n)] * value(cap_prod[K, n])
    end

    # 7) Capacity investment: storage
    for e in 1:K, n in N[e]
        CAPEX_OPEX += capacity_cost_storage[(e, n)] * value(cap_store[e, n])
    end
    for e in K+1:E_max, n in N[e]
        CAPEX_OPEX += capacity_cost_storage[(e, n)] * value(cap_store[e, n])
    end

    CAPEX_OPEX += production_cost
    ...
    data_out = DataFrame(
        ...
        Prod_Cost=prod_cost,
        Production_2_1_tonnes=prod_ts[(2, 1)],
        Production_2_2_tonnes=prod_ts[(2, 2)],
        Production_2_3_tonnes=prod_ts[(2, 3)],
        Production_2_4_tonnes=prod_ts[(2, 4)],
        Production_3_1_tonnes=prod_ts[(3, 1)],
        Production_3_2_tonnes=prod_ts[(3, 2)],
        ...
        Inflow_5_5_OPC=inflow_5_5_OPC,
        Outflow_2_1_tonnes=outflow_2[1],
        Outflow_2_2_tonnes=outflow_2[2],
        Outflow_2_3_tonnes=outflow_2[3],
        Outflow_2_4_tonnes=outflow_2[4],
        Outflow_3_1_tonnes=outflow_3[1],
        Outflow_3_2_tonnes=outflow_3[2],
        Outflow_4_2_tonnes=outflow_4_2,
        Outflow_5_5_tonnes=outflow_5_5,
        Production_Cost=CAPEX_OPEX,
        OPEX_Cost=production_cost,
        ...
        cap_prod_2_1=value(cap_prod[2, 1]),
        cap_prod_2_2=value(cap_prod[2, 2]),
        cap_prod_2_3=value(cap_prod[2, 3]),
        cap_prod_2_4=value(cap_prod[2, 4]),
        cap_prod_3_1=value(cap_prod[3, 1]),
        cap_prod_3_2=value(cap_prod[3, 2]),
        cap_store_1_1=value(cap_store[1, 1]),
        cap_store_1_2=value(cap_store[1, 2]),
        cap_store_1_3=value(cap_store[1, 3]),
        cap_store_1_4=value(cap_store[1, 4]),
        cap_store_1_5=value(cap_store[1, 5]),
        cap_store_2_1=value(cap_store[2, 1]),
        cap_store_2_2=value(cap_store[2, 2]),
        cap_store_2_3=value(cap_store[2, 3]),
        cap_store_2_4=value(cap_store[2, 4]),
        cap_store_3_1=value(cap_store[3, 1]),
        cap_store_3_2=value(cap_store[3, 2]),
        cap_store_4_1=value(cap_store[4, 1]),
        cap_store_4_2=value(cap_store[4, 2]),
        cap_store_4_3=value(cap_store[4, 3]),
        cap_store_4_4=value(cap_store[4, 4]),
        cap_store_4_5=value(cap_store[4, 5]),
        cap_store_5_1=value(cap_store[5, 1]),
        cap_store_5_5=value(cap_store[5, 5])
    )

    for ((e, n), energy_series) in energy_by_node
        colname = Symbol("Energy_$(e)_$(n)_kWh")
        data_out[!, colname] = energy_series
    end

    mkpath("capex_problem/results")
    out_file = "capex_problem/results/capex_opex_$(tag).xlsx"
    XLSX.writetable(out_file, Tables.columntable(data_out); sheetname="Series")
    println("✓ Scenario $(tag) saved → $(out_file)")
end

# Example of how to call the function with different input files:
